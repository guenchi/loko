...
(define (pci-scan-host proc)
  ...
  (define (new-device bus slot func)
    (define (device-header-layout sel)
      (fxbit-field (get-conf-u8 sel PCI-CFG-HEADER-TYPE) 0 7))
    (let ((dev (make-selector bus slot func)))
      (when (eqv? (device-header-layout dev) PCI-LAYOUT-BRIDGE)
        (set! *highest-bus* (fxmax *highest-bus* (pci-get-u8 dev PCI-CFG-01-SUBORDINATE-BUS))))
      (proc bus slot func)))
